...
通过自定义的 4 层 CNN 架构，结合批量归一化（Batch Normalization）、Dropout 随机失活以及丰富的数据增强技术，项目在未加载任何预训练权重的情况下，在 Kaggle 经典猫狗测试集上达到了 **92.0% 的准确率**。
...
*   [CNN_cat_vs_dog/](file:///f:/PyProject/CNN_cat_vs_dog) - **项目根目录**
    ...
    *   📁 [models/](file:///f:/PyProject/CNN_cat_vs_dog/models) - 模型权重存放目录
        *   `best_catdog_model.pth` - 已训练好的最佳模型权重（用于测试）
...
graph TD
    ...
    Flatten --> FC1["Linear (12544 -> 256) + ReLU + Dropout (0.15)"]
...
2.  **批归一化与失活（BatchNorm & Dropout 空间隔离与拟合释放）**：
    *   **架构定义**：在特征提取器（卷积层）部分引入 `BatchNorm2d` 稳定数据分布并加速收敛；在分类器部分，默认已将 `Dropout` 设为 `0.0`（关闭）。
    *   **调优结论**：经过多维超参数对比实验诊断，对于从头训练且无预训练的浅层网络，BatchNorm 与 Dropout 同时开启会产生极强的联合正则化约束（即强正则重叠），大大拉低了浅层网络在开始阶段对图像几何特征的学习效率，导致 Loss 在 0.693 锁死。将其关闭可瞬间释放模型拟合力，保证模型快速且正常起步。
3.  **学习率调度器（LR Scheduler）**：
    *   使用 Adam 优化器，初始学习率已调优下调至 **`0.0005`**。
    *   **调优结论**：在 25,000 张图的完整数据集下，初始 0.0025 的高学习率容易导致 Adam 优化器在前几个 Batch 的大梯度下破坏模型初始权重空间。调整为 0.0005 能够保证高吞吐状态下极稳健地温和启动和精确拟合，并配合 `StepLR` 每 15 轮自动衰减为原来的 `0.75`。
4.  **高吞吐与 GPU 性能加速（I/O 深度优化）**：
...
```
*   **训练轮次**：默认设为 60 Epochs。
*   **保存机制**：每当在验证集上获得更好的准确率时，最新的模型权重会自动更新保存到 `models/best_catdog_model-v2.pth`。
...
python test.py
```
*   **权重载入**：自动加载 `models/best_catdog_model.pth` 进行前向计算。

---

## 📈 模型评估指标

根据历史训练记录，该手写 CNN 模型的性能指标表现优异：
